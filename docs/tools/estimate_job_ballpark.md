...
This is a pricing-only tool. Scheduling never depends on it: when a duration is needed for availability, booking, or rescheduling, the workflow resolves it from the Services catalog on its own (see [get_availability](get_availability.md)).
...
- Call only when the customer asks about price, cost, or timing. Never call it just to obtain a duration.
